...
export default async function decorate(block) {
  const levels = /levels-(\d+)/.exec(block.className)[1];
  ...
  [...headings].forEach((title) => {
    if (hTagNames.indexOf(title.tagName) >= hTagNames.indexOf(firstHTagName) && hTagNames.indexOf(title.tagName) < (hTagNames.indexOf(firstHTagName) + Number(levels))) {
